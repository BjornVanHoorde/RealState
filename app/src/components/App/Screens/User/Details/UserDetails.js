...
const UserDetails = () => {
  ...
  const { user, onUpdate, onDelete } = useOutletContext();
    ...
    <ProfileContainer>
      <Row>
        ...
        <Col size="4" className="text-end">
          <Button>{t("users.edit.title")}</Button>
          <DeleteButton scope="users" id={user.id} onSuccess={onDelete} color="link">{t("users.delete.title")}</DeleteButton>
        </Col>
      </Row>
    </ProfileContainer>
  );
